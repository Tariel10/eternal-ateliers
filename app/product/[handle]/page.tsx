...
export default async function ProductDetail({ params }: ProductPageProps) {
  // GraphQL‑fetch
  const { data } = await shopifyFetch<{ product: any }>(GET_PRODUCT_BY_HANDLE, {
    ...
    <article className="max-w-5xl mx-auto py-16 px-4 grid lg:grid-cols-2 gap-12">
      ...
      <div>
        ...
        <p className="mt-6 text-gold font-semibold text-xl">
          {Number(product.variants?.nodes?.[0]?.price?.amount ?? 0).toLocaleString(
            'nl-NL',
